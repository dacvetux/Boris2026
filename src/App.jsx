...
const socialLinks = [
  {
    label: 'Facebook',
    href: LINKS.facebook,
    className: 'bg-[#24377e] text-white',
  },
  {
    label: 'Instagram',
    href: LINKS.instagram,
    className: 'bg-[#e32238] text-white',
  },
  {
    label: 'Telegram',
    href: LINKS.telegram,
    className: 'bg-[#90a8c2] text-[#0f1535]',
  },
];
      ...
      <header className="relative overflow-hidden border-b border-white/10 section-dot-bg">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-6 pb-14 pt-16 md:grid-cols-[1.1fr_.9fr] md:items-center md:pt-15">
          <div className="z-10">
            ...
            <div className="mt-8 inline-flex rounded-full bg-[#e32238] px-6 py-4 text-xl font-black uppercase leading-none text-white shadow-2xl sm:text-2xl md:text-3xl">
              Volitve 22.3.
            </div>
          </div>
      ...
      <main>
                ...
                <div className="space-y-4">
                  {socialLinks.map((item) => (
                    <a
                      key={item.label}
                      href={item.href}
                      className={`block rounded-2xl px-6 py-5 text-center text-xl font-black uppercase tracking-wide shadow-xl transition hover:scale-[1.01] md:text-2xl ${item.className}`}
                    >
                      {item.label}
                    </a>
                  ))}
        ...
        <section className="relative overflow-hidden bg-[#9fb9d3] text-[#24377e]">
          ...
          <div className="relative mx-auto max-w-6xl px-6 py-24 text-center md:py-24">
            ...
            <p className="mx-auto mt-6 max-w-5xl text-2xl font-black uppercase leading-tight md:text-5xl">
              In zakaj bi ga moral poznati
            </p>
            <div className="mx-auto mt-4 inline-block bg-[#24377e] px-5 py-3 text-3xl font-black uppercase text-white md:text-6xl">
              Vsak Mariborčan?
            </div>
          </div>
        </section>
              ...
              <div className="mt-10 max-w-xl rotate-[-2deg] bg-[#f4f0e8] p-6 text-[#111827] shadow-2xl">
                <ul className="space-y-4 text-xl font-medium leading-snug md:text-2xl">
                  {profileFacts.map((fact) => (
                    <li
                      key={fact}
                      className="border-b border-[#24377e]/30 pb-2 last:border-b-0"
                    >
                      • {fact}
                    </li>
                  ))}
          ...
          <div className="mx-auto max-w-6xl px-6 py-24 md:py-20">
            <div className="grid gap-5 md:grid-cols-2">
              {roles.map((item, index) => (
                <div
                  key={item}
                  className={`p-5 text-2xl font-black uppercase leading-tight shadow-xl md:text-2xl ${index === 3 || index === 4 ? 'bg-[#e32238] text-white' : 'bg-[#173564] text-white'}`}
                >
                  {item}
                </div>
              ))}
          ...
          <div className="mx-auto max-w-5xl px-6 py-24 text-center md:py-24">
            ...
            <div className="mx-auto mt-10 max-w-4xl space-y-5">
              {mandates.map((item, index) => (
                <div
                  key={item}
                  className="text-3xl font-black uppercase leading-tight md:text-4xl"
                >
                  <span
                    className={`${index % 2 === 0 ? 'bg-[#b0e7b8] text-[#14385f]' : 'text-[#90a8c2]'} inline-block px-4 py-2`}
                  >
                    {item}
                  </span>
                  {index !== mandates.length - 1 && (
                    <span className="ml-3 text-[#8cd7a2]">✓</span>
                  )}
                </div>
              ))}
            </div>
            <div className="mt-12 text-4xl font-black uppercase text-[#8cd7a2] md:text-5xl">
              Renesansa
            </div>
            <div className="text-4xl font-black uppercase text-white md:text-5xl">
              Štajerske metropole
            </div>
          </div>
        </section>
            ...
            <div className="mt-12 grid gap-8 lg:grid-cols-2">
              <div className="space-y-4">
                {todayStatsOne.map((item, idx) => (
                  <div
                    key={item}
                    className={`text-2xl font-black uppercase leading-tight md:text-2xl ${idx === 0 ? 'text-[#b9a8b6]' : 'text-white'}`}
                  >
                    <span
                      className={`${idx === 2 ? 'bg-[#00a98f]' : idx > 2 ? 'bg-[#e32238]' : idx === 1 ? 'bg-[#e32238]' : ''} inline-block px-3 py-2`}
                    >
                      {item}
                    </span>
                  </div>
              ...
              <div className="space-y-6">
                {todayStatsTwo.map((item) => (
                  <div
                    key={item}
                    className="text-2xl font-black uppercase leading-tight md:text-4xl"
                  >
                    <span className="inline-block bg-[#e32238] px-3 py-2 text-white">
                      {item}
                    </span>
                  </div>
                ))}
